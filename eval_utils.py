...
from cluster_methods import querybykmeans, querybyumap, querybyimageumap,queryby_umap_uni
...
def build_dataset(ds, class_map, num_classes):
    ...
    num_samples = len(ds)
    sample0 = ds[0][0]
    images_all = torch.empty((num_samples, *sample0.shape), dtype=sample0.dtype)
    labels_all = torch.empty(num_samples, dtype=torch.long)
    ...
    for i in tqdm(range(num_samples)):
        ...
        if isinstance(label, torch.Tensor):
            ...
            label_val = int(label)
        mapped_label = class_map[label_val]
        labels_all[i] = mapped_label
        
       
        indices_class[mapped_label].append(i)
    ...
    indices_class = [np.array(indices) for indices in indices_class]
...
def choose_dataset(images_all, labels_all, indices_class, num_classes, args):
    ...
    for n in range(num_classes):
        ...
        elif args.cluster == 'umap_uni':
            n_idx = queryby_umap_uni(images_all[indices_class[n]], num_classes, args)
