...
from datasets import MedSyn,MedSynFea
...
def get_dataset(dataset, data_path, batch_size=1, res=None, args=None):
    ...
    if dataset.startswith("medreal"):
        ...
        transforms.Resize(res, interpolation=Image.LANCZOS),
    ...
    elif dataset.startswith("medsynfea"):
        print(MedSynFea)
        channel = 3
        im_size = (res, res)
        num_classes = 9
        ...
        dst_train = MedSynFea(data_path, transform=transform)
        loader_train_dict = torch.utils.data.DataLoader(dst_train, batch_size=batch_size, shuffle=True, num_workers=16)
